...
function chooseSound(n) {
    ...
    //  E.g. if n=1 then sound = 0000. Sound format "ABCD" -> A: Intention - B: Gender - C: Length - D: Feature
    if (n <= 12) { sound += "0"} else { sound += "1" };
    if (n <= 6 || (13 <= n && n <= 18)) { sound += "0" } else { sound += "1" };
    if (n <= 3 || (7 <= n && n <= 9) || (13 <= n && n <= 15) || (19 <= n && n <= 21)) { sound += "0" } else { sound += "1" };
    switch (n % 3) {
        case 1:
            sound += "0"; break;
        case 2:
...
function setScreenLayout(enterDoor) {
    ...
    setTimeout(function () {
        ...
        if (sound[0] == 0) { setPlayer("yes") } else { setPlayer("no") } // these are just test sounds
        /*
        setPlayer(sound);
        */
...
function submit(enterDoor) {
    readSound(sound)
    localStorage.setItem(sound, + !enterDoor)
    
    // Increase score if player guessed robot intention correctly
    if (sound[0] != enterDoor) { score += 1 }
...
function complete() {
    ...
    setTimeout(function () {
        ...
        $("#container").append('<p class="score shadow2"> Your score is <font style="font-weight: bold" color="gold">' + score + '</font> out of 24.</p>')
